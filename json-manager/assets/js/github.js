const TOKEN_KEY = "json_tupa_token";
...
async function upsertRepoFile(owner, repo, path, content, sha, message, branch) {
  return githubRequest(`/repos/${owner}/${repo}/contents/${path}`, {
    method: "PUT",
    body: {
      message,
...
async function listWorkflows(owner, repo) {
  return githubRequest(`/repos/${owner}/${repo}/actions/workflows`);
}

async function dispatchWorkflow(owner, repo, workflowId, ref) {
